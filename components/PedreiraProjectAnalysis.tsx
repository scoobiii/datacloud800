...
const PedreiraProjectAnalysis: React.FC = () => {
    ...
    const FinancialInfoCard: React.FC<{title: string; capex: string; opex: string; contribution: string; icon: React.ReactNode; children?: React.ReactNode}> = ({ title, capex, opex, contribution, icon, children }) => (
        <DashboardCard title={title} icon={icon}>
            <div className="space-y-4">
                ...
                {children && <div className="border-t border-gray-700 pt-4 mt-4 text-gray-300">{children}</div>}
            </div>
        </DashboardCard>
    );
        ...
        <div className="space-y-6">
            ...
            <DashboardCard title="Fluxograma do Processo Integrado" icon={<ActivityIcon className="w-6 h-6" />}>
                <div className="p-4 bg-gray-900 rounded-lg overflow-x-auto">
                    <div className="mermaid text-center" style={{ minWidth: '800px' }}>
                        {mermaidGraph}
                    </div>
                </div>
            </DashboardCard>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    >
                        <p>Este é o coração do sistema de trigeração. Sua função é converter calor residual (gratuito) em frio (valioso). Transforma um passivo (calor) em um ativo (refrigeração) que será "vendido" internamente para os outros sistemas (TIAC, Fogging, Datacenter).</p>
                    </FinancialInfoCard>
                    ...
                    >
                        <p>Usa o frio do chiller para garantir que a turbina opere sempre em sua máxima potência de base, independentemente do clima. Evita a perda de centenas de MW em dias quentes, garantindo a entrega da capacidade máxima quando a demanda (e o preço) é maior.</p>
                    </FinancialInfoCard>
                    ...
                    >
                        <p>Injeta uma névoa ultrafina para dar um "boost" de potência, aumentando a geração para além da capacidade nominal. Gera centenas de MW adicionais "on-demand", permitindo capturar picos de preço no mercado ou cumprir metas de despacho mais altas.</p>
                    </FinancialInfoCard>
                </div>
            </div>
...
export default PedreiraProjectAnalysis;